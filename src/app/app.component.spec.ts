...
import {
  ...
  utilityServiceMock,
  splashcreenTelemetryActionHandlerDelegateMock,
  splashscreenImportActionHandlerDelegateMock,
  splaschreenDeeplinkActionHandlerDelegateMock,
  appHeaderServiceMock,
  logoutHandlerServiceMock,
  networkMock,
  appRatingServiceMock,
  activePageServiceMock,
  notificationMock
...
describe('CollectionDetailsPage Component', () => {
  ...
  beforeEach(() => {
    ...
    platformMock.ready.mockResolvedValue('');
    ...
    myApp = new MyApp(
      ...
      splashcreenTelemetryActionHandlerDelegateMock as any,
      splashscreenImportActionHandlerDelegateMock as any,
      splaschreenDeeplinkActionHandlerDelegateMock as any,
      appHeaderServiceMock as any,
      logoutHandlerServiceMock as any,
      networkMock as any,
      appRatingServiceMock as any,
      activePageServiceMock as any,
  ...
  it('should be call ngOnInit()', () => {
    // arrange
    appHeaderServiceMock.headerConfigEmitted$ = Observable.from([{ headerConfig: 'myApp' }]) as any;
    commonUtilServiceMock.networkAvailability$ = Observable.from([{ available: true }]) as any;
    appMock.getActiveNavs.mockReturnValue([{ getActive: jest.fn(() => ({ instance: CoursesPage })) }]);
    notificationMock.setupLocalNotification.mockReturnValue('');
    // act
    myApp.ngOnInit();
    // expect(myApp.addNetworkTelemetry).toHaveBeenCalledWith('internet-connected', 'pageId');
  });

  it('should be call addNeworkTelemetry() when ngOnInit() if network unavailable', () => {
    // arrange
    appHeaderServiceMock.headerConfigEmitted$ = Observable.from([{ headerConfig: 'myApp' }]) as any;
    commonUtilServiceMock.networkAvailability$ = Observable.from([{ available: false }]) as any;
    appMock.getActiveNavs.mockReturnValue([{ getActive: jest.fn(() => ({ instance: CoursesPage })) }]);
    notificationMock.setupLocalNotification.mockReturnValue('');
    // act
    myApp.ngOnInit();
  ...
  it('should be call ngAfterViewInit()', () => {
    // arrange
    platformMock.resume = Observable.from([{ name: 'resume' }]) as any;
    telemetryGeneratorServiceMock.generateInterruptTelemetry.mockReturnValue('');
    platformMock.pause = Observable.from([{ name: 'resume' }]) as any;
    // act
    myApp.ngAfterViewInit();
    // assert
  });

  it('should be call handleBackButton()', () => {
    // arrange
    platformMock.registerBackButtonAction.mockReturnValue(jest.fn());
    appMock.getRootNavs.mockReturnValue([{ getActive: () => ({name: 'myApp'}), canGoBack: jest.fn() }]);
    console.log(appMock.getRootNavs()[0]);
    myApp.nav = {
       getActive: jest.fn(() => ({ instance: TabsPage }))
    };
    // appMock.getRootNavs.mockReturnValue([{canGoBack: jest.fn()}]);

    // act
    myApp.handleBackButton();
    platformMock.registerBackButtonAction.mock.calls[0][0].call(myApp);
    // assert
  });

  it('should be call  generateNetworkTelemetry()', () => {
    // arrange

    // act
    myApp.generateNetworkTelemetry();
    // assert
  });

  it('should be called getUtmParameter() for utm data', () => {
    // arrange
    utilityServiceMock.getUtmInfo.mockResolvedValue('utmdata');
    telemetryGeneratorServiceMock.generateInteractTelemetry.mockReturnValue('utmdata');
    // act
    myApp.getUtmParameter();
  });
});
