...
import { ReportService, ServiceProvider, ContentService, TelemetryService, AuthService, ProfileService, SharedPreferences, BuildParamService, FrameworkService } from "sunbird";
...
import { LoadingControllerMock, NavParamsMock , NavMock,AppMock,DeepLinkerMock,AuthServiceMock } from "../../../../test-config/mocks-ionic";
...
describe("GroupReportListPage", () => {
    ...
    beforeEach(() => {
        ...
        TestBed.configureTestingModule({
            ...
            providers: [AppGlobalService, ReportService, TelemetryGeneratorService,ReportSummary, PageId, Environment, InteractType, InteractSubtype,
                Events,PopoverController,ServiceProvider,ContentService,TelemetryService,ProfileService,SharedPreferences,Config,BuildParamService,
                FrameworkService,
    ...
    describe('fetchAssessment should be executed,', function(){
        ...
        it("#when called with users event and false args with fromUserAssessment object undefined, if block should be invoked",(done)=>{
            ...
            spyOn(reportService, 'getReportsByUser').and.callFake(function ({ }, success, error) {
                let data = JSON.stringify((mockres.getReportsByUser))
                return success(data);
            });
            comp.fetchAssessment('users',false);
        ...
        it("#when called with 'users event and false' args with error response from reportService, property fromUserAssessment should remain undefined",(done)=>{
            
            const reportService = TestBed.get(ReportService);
           
            spyOn(comp, 'fetchAssessment').and.callThrough();
            spyOn(reportService, 'getReportsByUser').and.callFake(function ({ }, success, error) {
                let data = JSON.stringify({error:"error for testing "})
                return error(data);
            });
            comp.fetchAssessment('users',false);
            expect(comp.fetchAssessment).toHaveBeenCalled();
        ...
        it("#when called with questions event and false args, else block should be invoked",(done)=>{
            ...
            spyOn(comp, 'fetchAssessment').and.callThrough();
            spyOn(reportService, 'getReportsByQuestion').and.callFake(function ({ }, success, error) {
                let data = JSON.stringify((mockres.getReportsByQuestion));
                return success(data);
            });
        ...
        it("#when called with questions event and false args for meeting elseif, with error response from reportService, property getReportsByQuestion should remain undefined",(done)=>{
            //TODO call fetchAssessment check if block invoked
            const reportService = TestBed.get(ReportService);
